...
| `/slowsql` | 慢 SQL（默认 >1s） |
| `/sqlfetch <SQL_ID>` | **v1.1.28 新增** · 按 SQL_ID 拉可 EXPLAIN 的完整 SQL（带 schema），喂给 sqltune |
...
| openGauss `gs_stat_activity` 报缺视图 | og-lite 无此视图；v1.1.28 已自动跳过 |
