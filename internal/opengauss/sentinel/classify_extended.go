 ...
 *	  Anomaly classifier (extended) for openGauss Sentinel — turns raw
 ...
 *
 * IDENTIFICATION
 *	  internal/opengauss/sentinel/classify_extended.go
 *
 *-------------------------------------------------------------------------
 */
package sentinel
...
// Adapted from PG extended rules with OpenGauss-specific adjustments.
...
// ── Rule 14: Cache + slow query compound ──
...
// ── Rule 18: Connection + lock compound ──
...
// ── Rule 21: Replication + WAL compound ──
...
func classifyMultipleSlowSQLs(r BurstReport) Classification {
	...
	return Classification{
		...
		Confidence: confidence,
		Evidence: []string{
			fmt.Sprintf("%d 个 SQL 最大耗时超过 1 秒", slowCount),
			"多个不同 SQL 同时变慢",
		},
	}
}
