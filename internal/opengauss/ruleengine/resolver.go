 ...
 *
 * IDENTIFICATION
 *	  internal/opengauss/ruleengine/resolver.go
 *
 *-------------------------------------------------------------------------
 */
package ruleengine
...
//
// Given N diagnosis results from fired rules, distill them to at most 1-2
// root causes. Everything else is absorbed as downstream symptoms.
...
func Resolve(results []*Diagnosis, rules []*Rule) *DiagOutput {
	...
	// Enrich each diagnosis with cross-reference findings.
...
// analyzeCausalChain identifies upstream/downstream relationships.
...
// convergeToCoreCause scores, ranks, and selects 1-2 core causes.
...
// enrichAbsorbed collects names of absorbed diagnoses.
