 ...
 *	  database diagnosis engine for OpenGauss. It works without LLM and
 ...
 *
 * IDENTIFICATION
 *	  internal/opengauss/ruleengine/types.go
 *
 *-------------------------------------------------------------------------
 */
// Package ruleengine provides a deterministic, decision-tree-based database
// diagnosis engine for OpenGauss. It works without LLM and serves as fallback
...
const (
	SignalWaitEvent SignalType = iota // Wait event name
	SignalErrorCode                   // Error code
...
type Signal struct {
	Type SignalType
	Key  string // e.g., "lock_waits", "active_sessions"
}
...
type Rule struct {
	ID       string
	Name     string
	Category string // "vacuum" / "lock" / "wal" / "connection" / "replication" / "sql_perf" / "memory" / "checkpoint"
