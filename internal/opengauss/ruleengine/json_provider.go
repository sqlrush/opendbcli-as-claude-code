 ...
 *
 * IDENTIFICATION
 *	  internal/opengauss/ruleengine/json_provider.go
 *
 *-------------------------------------------------------------------------
 */
package ruleengine
...
func NewJSONRuleProvider(fsys fs.FS, version string) *JSONRuleProvider {
	...
	if err != nil {
		log.Printf("[og-json-provider] failed to read directory: %v", err)
		return p
	}
...
func NewJSONRuleProviderFromDir(dir string, version string) *JSONRuleProvider {
	...
	if err != nil {
		log.Printf("[og-json-provider] failed to glob directory: %v", err)
		return p
	}
...
// NewJSONRuleProviderFromEmbedded creates a provider from the embedded OpenGauss JSON rules.
