...
func (s *WDRAnalyzeSkill) Description() string {
	return "解读 WDR 报告 + 风险分级 + Top SQL 优化建议"
}
...
func (s *WDRAnalyzeSkill) Execute(ctx context.Context, params skill.Params) (*skill.Result, error) {
	...
	if reportPath, err := reportOutputPath(opts.OutputDir, analysis); err == nil {
	...
	// Phase 7: Persist. The report path is already rendered in the footer when known,
	// so terminal output and saved markdown stay identical.
	if reportPath, err := persistReport(opts.OutputDir, analysis, markdown); err == nil && analysis.ReportPath == "" {
		analysis.ReportPath = reportPath
	}

	// Engine-level tool orchestration already treats wdranalyze results as
	// passthrough by tool name. Keep the skill output itself free of hidden
	// HTML directives so slash, natural-language, and saved reports match.
	rendered := markdown
...
// reportOutputPath returns the deterministic target path for a WDR markdown report.
func reportOutputPath(outputDir string, a *wdranalyze.Analysis) (string, error) {
	if outputDir == "" {
...
func persistReport(outputDir string, a *wdranalyze.Analysis, markdown string) (string, error) {
	path := strings.TrimSpace(a.ReportPath)
	if path == "" {
		var err error
		path, err = reportOutputPath(outputDir, a)
		if err != nil {
			return "", err
		}
	}
