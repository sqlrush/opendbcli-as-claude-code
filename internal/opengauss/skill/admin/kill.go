 ...
 *
 * IDENTIFICATION
 *	  internal/opengauss/skill/admin/kill.go
 *
 *-------------------------------------------------------------------------
 */
package admin
...
const ogSessionDetailSQL = `SELECT pid, usename, client_addr::text, datname, state,
       EXTRACT(EPOCH FROM (now() - backend_start))::int AS uptime_sec,
       CASE WHEN waiting THEN 'waiting' ELSE '' END AS wait_type,
       CASE WHEN waiting THEN 'lock' ELSE '' END AS wait_event,
...
func (s *KillSkill) ToolDef() skill.ToolDef {
	return skill.ToolDef{
		Name:        "kill",
		Description: "Terminate an OpenGauss backend or cancel its current query",
...
func (s *KillSkill) Execute(ctx context.Context, params skill.Params) (*skill.Result, error) {
	...
	pid, mode, confirm := parseOGKillArgs(raw)
	if pid == "" {
...
func (s *KillSkill) showConfirmation(ctx context.Context, pid, mode string) (*skill.Result, error) {
	...
	query := fmt.Sprintf(ogSessionDetailSQL, pid)
	detail, err := s.driver.Query(ctx, query)
	if err != nil {
		msg := fmt.Sprintf("查询会话信息: %v", err)
	...
	row := detail.Rows[0]
	user := ogValOrDash(row, 1)
	clientAddr := ogValOrDash(row, 2)
	dbName := ogValOrDash(row, 3)
	state := ogValOrDash(row, 4)
	uptimeSec := ogValOrDash(row, 5)
	waitType := ogValOrDash(row, 6)
	waitEvent := ogValOrDash(row, 7)
	queryText := ogValOrDash(row, 8)
...
// parseOGKillArgs extracts pid, mode (cancel), and confirm flag from args.
func parseOGKillArgs(args string) (pid, mode string, confirm bool) {
...
// ogValOrDash returns the string value at index i, or "-" if nil/empty.
func ogValOrDash(row []any, i int) string {
