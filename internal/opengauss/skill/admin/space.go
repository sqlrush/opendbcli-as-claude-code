 ...
 *
 * IDENTIFICATION
 *	  internal/opengauss/skill/admin/space.go
 *
 *-------------------------------------------------------------------------
 */
package admin
...
func (s *SpaceSkill) Execute(ctx context.Context, _ skill.Params) (*skill.Result, error) {
	...
	rendered := formatOGSpacePanel(result)
	return &skill.Result{
...
func formatOGSpacePanel(qr *db.QueryResult) string {
	...
	header := " " + format.PadRight("Database", ogSpNameW) + " " +
		format.PadRight("Size", ogSpSizeW) + "  " +
		format.PadRight("Bar", ogSpBarW)
	...
	for _, row := range qr.Rows {
		...
		bar := format.ProgressBar(pct, ogSpBarW)

		line := " " + format.PadRight(format.TruncDisplayWidth(name, ogSpNameW), ogSpNameW) + " " +
			format.PadRight(sizeStr, ogSpSizeW) + "  " + bar
