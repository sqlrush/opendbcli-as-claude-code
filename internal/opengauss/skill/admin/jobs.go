/*-------------------------------------------------------------------------
 *
 * jobs.go
 *	  JobsSkill shows pg_cron scheduled jobs.
 *
 *
 * Copyright 2026 Sqlrush <sqlrush@gmail.com>
 *
 * Author: Sqlrush <sqlrush@gmail.com>
 *
 * IDENTIFICATION
 *	  internal/opengauss/skill/admin/jobs.go
 *
 *-------------------------------------------------------------------------
 */
package admin

import (
	"context"
	"fmt"

	"github.com/sqlrush/opendb/internal/db"
	"github.com/sqlrush/opendb/internal/skill"
)

const jobsSQL = `SELECT
  jobid,
  schedule,
  command,
  nodename,
  database,
  username,
  active
FROM cron.job
ORDER BY jobid`

// JobsSkill shows pg_cron scheduled jobs.
type JobsSkill struct {
	driver db.Driver
}

// NewJobsSkill creates a JobsSkill backed by the given driver.
func NewJobsSkill(driver db.Driver) *JobsSkill {
	return &JobsSkill{driver: driver}
}

func (s *JobsSkill) Name() string                       { return "jobs" }
func (s *JobsSkill) Description() string                { return "定时任务 (pg_cron)" }
func (s *JobsSkill) SecurityLevel() skill.SecurityLevel { return skill.LevelReadOnly }
func (s *JobsSkill) Validate(_ skill.Params) error      { return nil }

func (s *JobsSkill) CLIDef() skill.CLIDef {
	return skill.CLIDef{Usage: "/jobs", Examples: []string{"/jobs"}}
}

func (s *JobsSkill) ToolDef() skill.ToolDef {
	return skill.ToolDef{
		Name:        "jobs",
		Description: "List pg_cron scheduled jobs (requires pg_cron extension)",
	}
}

func (s *JobsSkill) Execute(ctx context.Context, _ skill.Params) (*skill.Result, error) {
	result, err := s.driver.Query(ctx, jobsSQL)
	if err != nil {
		// pg_cron extension may not be installed
		return &skill.Result{
			Type:     skill.ResultText,
			Rendered: "pg_cron 扩展未安装或无权限访问 cron.job 表",
			Summary:  "pg_cron not available",
		}, nil
	}
	if len(result.Rows) == 0 {
		return &skill.Result{
			Type:     skill.ResultText,
			Rendered: "无定时任务 (pg_cron)",
			Summary:  "no cron jobs",
		}, nil
	}
	return &skill.Result{
		Type:     skill.ResultTable,
		Data:     result,
		Rendered: fmt.Sprintf("定时任务 (pg_cron) — %d 个", len(result.Rows)),
		Summary:  fmt.Sprintf("%d 个定时任务", len(result.Rows)),
	}, nil
}
