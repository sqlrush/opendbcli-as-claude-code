 ...
 *
 * health.go
 *	  HealthSkill provides a OpenGauss database health overview.
 *
 *
 ...
 *
 * IDENTIFICATION
 *	  internal/opengauss/skill/monitor/health.go
 *
 *-------------------------------------------------------------------------
 */
package monitor
...
// healthMaintDeadTupSQL: dead tuples only — always works on OG/PG.
// Split from the original combined query because age(datfrozenxid) does not
// exist on OG 5.0 (datfrozenxid is xid32 there, no overload), and the old
// combined query returned an error → maintenanceSection got silently
// skipped → LLM saw "17 项全过" while dead_tup was actually millions. v1.1.12
const healthMaintDeadTupSQL = `SELECT COALESCE(sum(n_dead_tup), 0) AS total_dead_tup
FROM pg_stat_user_tables`

// healthMaintXidSQL: XID age using OG-compatible functions. txid_current()
// returns a bigint that works on both PG and OG; datfrozenxid is xid32 on OG
// so we cast through text→numeric. Result is the # of transactions since
// the database's frozen XID — proxy for wraparound risk.
const healthMaintXidSQL = `SELECT
  txid_current() - max(CAST(datfrozenxid::text AS numeric)) AS max_xid_age
FROM pg_database
...
func (s *HealthSkill) ToolDef() skill.ToolDef {
	return skill.ToolDef{
		Name:        "health",
		Description: "Show OpenGauss database health overview: connections, cache hit ratio, transaction rate, dead tuples, replication, XID age",
	}
}
...
func (s *HealthSkill) Execute(ctx context.Context, _ skill.Params) (*skill.Result, error) {
	...
	// Extensions list (absorbed from the removed /extensions skill).
	if sec, err := s.extensionsSection(ctx); err == nil {
		sections = append(sections, sec)
	}
	...
	rendered := format.Panel("OpenGauss Health", allSections)
	return &skill.Result{
...
func (s *HealthSkill) instanceSection(ctx context.Context) (format.PanelSection, []checkResult, error) {
	...
	row := result.Rows[0]
	version := extractOpenGaussVersion(asStr(row[0]))
...
func (s *HealthSkill) maintenanceSection(ctx context.Context) (format.PanelSection, []checkResult, error) {
	// Two queries instead of one: dead_tup query is portable; xid age query
	// uses OG-compatible math (no age() overload). If xid age query fails we
	// still emit the dead_tup check rather than silently dropping the whole
	// section — that's the bug v1.1.12 fixed.
	var checks []checkResult

	if r, err := s.driver.Query(ctx, healthMaintDeadTupSQL); err == nil && len(r.Rows) > 0 {
		deadTup := toFloat64Val(r.Rows[0][0])
		checks = append(checks, checkResult{
			"Dead Tuples",
			boolStatus(deadTup < 100000),
			fmt.Sprintf("%.0f", deadTup),
		})
	}

	if r, err := s.driver.Query(ctx, healthMaintXidSQL); err == nil && len(r.Rows) > 0 {
		xidAge := toFloat64Val(r.Rows[0][0])
		xidPct := safePct(xidAge, 2147483647)
		checks = append(checks, checkResult{
			"XID Age",
			boolStatus(xidPct < 50),
			fmt.Sprintf("%.0f (%.2f%% to wraparound)", xidAge, xidPct),
		})
	}
...
// extensionsSection enumerates installed extensions — folded in from the
// removed /extensions skill so /health keeps coverage.
const healthExtensionsSQL = `SELECT extname, extversion
FROM pg_extension
ORDER BY extname`

func (s *HealthSkill) extensionsSection(ctx context.Context) (format.PanelSection, error) {
	result, err := s.driver.Query(ctx, healthExtensionsSQL)
	if err != nil || result == nil || len(result.Rows) == 0 {
		return format.PanelSection{
			Header: "Extensions",
			Lines:  []string{"  (none installed or view unavailable)"},
		}, nil
	}
	lines := make([]string, 0, len(result.Rows))
	for _, r := range result.Rows {
		lines = append(lines, fmt.Sprintf("  %-24s %v", r[0], r[1]))
	}
	return format.PanelSection{
		Header: fmt.Sprintf("Extensions (%d)", len(result.Rows)),
		Lines:  lines,
	}, nil
}
