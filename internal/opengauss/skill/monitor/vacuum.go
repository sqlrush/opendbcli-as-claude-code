 ...
 *
 * IDENTIFICATION
 *	  internal/opengauss/skill/monitor/vacuum.go
 *
 *-------------------------------------------------------------------------
 */
package monitor
...
// vacuumSQL filters out OG internal schemas — otherwise WDR snapshot tables
// (which are 100% dead by design due to snapshot rotation) dominate the
// output and hide real user-table bloat.
var vacuumSQL = `SELECT
  schemaname, relname,
