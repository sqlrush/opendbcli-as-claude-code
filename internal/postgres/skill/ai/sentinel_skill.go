 ...
 *
 * IDENTIFICATION
 *	  internal/postgres/skill/ai/sentinel_skill.go
 *
 *-------------------------------------------------------------------------
 */
package ai
...
func (s *SentinelSkill) start(_ context.Context) (*skill.Result, error) {
	...
	return &skill.Result{
		...
		Rendered: fmt.Sprintf("PG Sentinel 哨兵模式已启动\n  探测: 15 项指标 (1s/10s/30s 三层探针)\n  触发: 自适应 3-sigma\n  基线: %d 样本\n  突发采集: 最长%v",
...
func (s *SentinelSkill) status() (*skill.Result, error) {
	...
	b.WriteString("PG Sentinel 哨兵状态\n\n")
