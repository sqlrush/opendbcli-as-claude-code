 ...
 *
 * rule_skill.go
 *	  RuleSkill runs deterministic rule-based classification on sentinel
 *	  reports. Works without LLM.
 *
 *
 ...
 *
 * IDENTIFICATION
 *	  internal/postgres/skill/ai/rule_skill.go
 *
 *-------------------------------------------------------------------------
 */
package ai
...
// RuleSkill runs deterministic rule-based classification on sentinel reports.
// Works without LLM.
...
func (s *RuleSkill) Description() string                { return "Rule-based PG diagnosis (no LLM)" }
func (s *RuleSkill) SecurityLevel() skill.SecurityLevel { return skill.LevelReadOnly }

func (s *RuleSkill) ToolDef() skill.ToolDef {
	return skill.ToolDef{
		Name:        "rule",
		Description: "Rule-based PostgreSQL diagnosis (no LLM required)",
	}
}
...
func (s *RuleSkill) Execute(ctx context.Context, params skill.Params) (*skill.Result, error) {
	...
	// Classify (sentinel simple classification).
	classification := sentinel.Classify(*report)
	report.Classification = classification

	// Run rule engine (decision-tree-based deep diagnosis).
...
// SQL for building live report from current PG state.
const (
...
FROM pg_stat_activity`
	...
	liveBlockerSQL = `SELECT COUNT(DISTINCT pid) FROM pg_stat_activity
WHERE pid = ANY(
  SELECT unnest(pg_blocking_pids(pid)) FROM pg_stat_activity WHERE wait_event_type = 'Lock'
)`
)

// buildLiveReport queries current PG state and constructs a BurstReport.
...
func (s *RuleSkill) collectExtendedMetrics(ctx context.Context, report *sentinel.BurstReport) {
	// Dead tuple ratio + cache hit from pg_stat_user_tables / pg_stat_database.
	dbStatsSQL := `SELECT
...
FROM pg_stat_user_tables`
	...
	if err == nil && len(result.Rows) > 0 && len(result.Rows[0]) >= 4 {
		row := result.Rows[0]
		deadRatio := toFloat(row[0])
		cacheHit := toFloat(row[1])
		xidAge := toFloat(row[2])
		bloated := toFloat(row[3])

		report.Metrics["dead_tuple_ratio"] = sentinel.MetricSummary{Avg: deadRatio, Max: deadRatio}
		report.Metrics[string(sentinel.MetricCacheHitPct)] = sentinel.MetricSummary{Avg: cacheHit, Max: cacheHit}
		report.Metrics[string(sentinel.MetricXIDAgeRatio)] = sentinel.MetricSummary{Avg: xidAge, Max: xidAge}
		report.Metrics["bloated_tables"] = sentinel.MetricSummary{Avg: bloated, Max: bloated}
	}

	// Temp files + seq scan heavy tables.
	perfSQL := `SELECT
  COALESCE((SELECT SUM(temp_files) FROM pg_stat_database WHERE datname = current_database()), 0) AS temp_files,
  COALESCE((SELECT SUM(temp_bytes) FROM pg_stat_database WHERE datname = current_database()), 0) AS temp_bytes,
  (SELECT COUNT(*) FROM pg_stat_user_tables WHERE seq_scan > 1000 AND seq_tup_read > 100000) AS seq_scan_heavy_tables
`
	result2, err := s.driver.Query(ctx, perfSQL)
	if err == nil && len(result2.Rows) > 0 && len(result2.Rows[0]) >= 3 {
		row := result2.Rows[0]
		report.Metrics["temp_files"] = sentinel.MetricSummary{Avg: toFloat(row[0]), Max: toFloat(row[0])}
		report.Metrics["temp_bytes"] = sentinel.MetricSummary{Avg: toFloat(row[1]), Max: toFloat(row[1])}
		report.Metrics["seq_scan_heavy_tables"] = sentinel.MetricSummary{Avg: toFloat(row[2]), Max: toFloat(row[2])}
	}
}
