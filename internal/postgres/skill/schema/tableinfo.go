 ...
 *
 * IDENTIFICATION
 *	  internal/postgres/skill/schema/tableinfo.go
 *
 *-------------------------------------------------------------------------
 */
package schema
...
// P08: Enhanced table stats SQL with TOAST table size.
const pgTableStatsSQL = `SELECT
  pg_total_relation_size('%s.%s'::regclass) / 1048576 AS total_mb,
  pg_table_size('%s.%s'::regclass) / 1048576 AS table_mb,
  pg_indexes_size('%s.%s'::regclass) / 1048576 AS index_mb,
  COALESCE((SELECT pg_relation_size(c.reltoastrelid) / 1048576
    FROM pg_class c WHERE c.oid = '%s.%s'::regclass AND c.reltoastrelid != 0), 0) AS toast_mb,
  (SELECT n_live_tup FROM pg_stat_user_tables WHERE schemaname = '%s' AND relname = '%s') AS live_tup,
  (SELECT n_dead_tup FROM pg_stat_user_tables WHERE schemaname = '%s' AND relname = '%s') AS dead_tup,
  (SELECT last_analyze::text FROM pg_stat_user_tables WHERE schemaname = '%s' AND relname = '%s') AS last_analyze,
  (SELECT last_autovacuum::text FROM pg_stat_user_tables WHERE schemaname = '%s' AND relname = '%s') AS last_autovacuum`
...
func (s *TableInfoSkill) Execute(ctx context.Context, params skill.Params) (*skill.Result, error) {
	...
	if statsResult != nil && len(statsResult.Rows) > 0 {
		...
		statsLines := []string{
			fmt.Sprintf("Total Size      : %s MB", asStr(row[0])),
			fmt.Sprintf("Table Size      : %s MB", asStr(row[1])),
			fmt.Sprintf("Index Size      : %s MB", asStr(row[2])),
		}
		// P08: Show TOAST table size if present.
		if len(row) > 3 {
			toastStr := asStr(row[3])
			if toastStr != "0" && toastStr != "" {
				statsLines = append(statsLines, fmt.Sprintf("TOAST Size      : %s MB", toastStr))
			}
		}
		statsLines = append(statsLines,
			fmt.Sprintf("Live Tuples     : %s", asStr(row[4])),
			fmt.Sprintf("Dead Tuples     : %s", asStr(row[5])),
			fmt.Sprintf("Last Analyze    : %s", asStr(row[6])),
			fmt.Sprintf("Last Autovacuum : %s", asStr(row[7])),
		)
