 ...
 *
 * IDENTIFICATION
 *	  internal/postgres/skill/admin/alert.go
 *
 *-------------------------------------------------------------------------
 */
package admin
...
func (s *AlertSkill) Execute(ctx context.Context, _ skill.Params) (*skill.Result, error) {
	...
	rendered := formatPGAlertPanel(result)
...
// pgAlertEntry holds one parsed pg_stat_database row.
type pgAlertEntry struct {
	datname    string
	conflicts  int64
	deadlocks  int64
	tempFiles  int64
...
// pgSeverityLabel returns the severity based on deadlock/conflict counts.
func pgSeverityLabel(e pgAlertEntry) (string, string) {
	if e.deadlocks > 0 {
...
// formatPGAlertPanel renders PG alert data with severity labels and compact layout.
func formatPGAlertPanel(qr *db.QueryResult) string {
	...
	entries := make([]pgAlertEntry, 0, len(qr.Rows))
	for _, row := range qr.Rows {
		if len(row) < 8 {
			continue
		}
		entries = append(entries, pgAlertEntry{
