 ...
 *
 * alter.go
 *	  AlterSkill modifies PostgreSQL system parameters via ALTER SYSTEM
 *	  SET.
 *
 *
 ...
 *
 * IDENTIFICATION
 *	  internal/postgres/skill/admin/alter.go
 *
 *-------------------------------------------------------------------------
 */
package admin
...
// AlterSkill modifies PostgreSQL system parameters via ALTER SYSTEM SET.
...
func (s *AlterSkill) ToolDef() skill.ToolDef {
	return skill.ToolDef{
		Name:        "alter",
		Description: "Alter a PostgreSQL system parameter via ALTER SYSTEM SET, or show current value",
...
func (s *AlterSkill) alterParam(ctx context.Context, paramName, paramValue string, info pgParamInfo) (*skill.Result, error) {
	...
	if info.context == "sighup" || info.context == "superuser" {
		...
		resultLines = append(resultLines,
			fmt.Sprintf("%s⚠ context=postmaster, 需重启 PostgreSQL 生效%s", ansiWarn, ansiReset))
	} else {
		resultLines = append(resultLines,
