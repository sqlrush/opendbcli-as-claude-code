 ...
 *
 * IDENTIFICATION
 *	  internal/postgres/skill/monitor/resource.go
 *
 *-------------------------------------------------------------------------
 */
package monitor
...
func (s *ResourceSkill) ToolDef() skill.ToolDef {
	return skill.ToolDef{
		Name:        "resource",
		Description: "Show PostgreSQL resource usage vs limits: connections, WAL senders, checkpoints, workers",
	}
}
...
func (s *ResourceSkill) Execute(ctx context.Context, _ skill.Params) (*skill.Result, error) {
	...
	if err == nil && len(workerResult.Rows) > 0 {
		...
		sections = append(sections, format.PanelSection{
			Header: "Workers",
			Lines: []string{
				fmt.Sprintf("Max Workers     : %s", asStr(row[0])),
				fmt.Sprintf("Max Parallel    : %s", asStr(row[1])),
