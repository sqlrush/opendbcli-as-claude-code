 ...
 *
 * collector.go
 *	  Collector queries PostgreSQL via db.Driver and assembles Snapshot
 *	  values.
 *
 *
 ...
 *
 * IDENTIFICATION
 *	  internal/postgres/monitor/dbtop/collector.go
 *
 *-------------------------------------------------------------------------
 */
package dbtop
...
// debugLog writes debug info to /tmp/opendb-pg-dbtop.log
func debugLog(format string, args ...any) {
	f, err := os.OpenFile("/tmp/opendb-pg-dbtop.log", os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	if err != nil {
		return
	}
	defer f.Close()
...
// SQL constants for PostgreSQL data dictionary queries.
const (
...
WHERE datname = current_database()`
	...
	stmtCallsSQL = `SELECT COALESCE(SUM(calls), 0) FROM pg_stat_statements WHERE dbid = (SELECT oid FROM pg_database WHERE datname = current_database())`

	walBytesSQL = `SELECT wal_bytes FROM pg_stat_wal`
...
LIMIT 10`
	...
	// Fallback SQL for PG < 14 (no query_id column).
	activeSessionsSQLNoQID = `SELECT
  pid, usename,
  '' AS query_id,
  COALESCE(wait_event_type, 'CPU') AS wait_type,
  COALESCE(wait_event, 'On CPU') AS event,
...
// Collector queries PostgreSQL via db.Driver and assembles Snapshot values.
type Collector struct {
	...
	hasQueryID     *bool // nil = unknown, true/false after first check (PG14+)
	hasPGStatStmts *bool // nil = unknown, true/false after first check
}
...
// Collect runs all PostgreSQL queries in parallel and returns a Snapshot.
func (c *Collector) Collect(ctx context.Context) Snapshot {
	...
	odberr.SafeGo(odberr.ErrSentinelCollect, func() {
		...
		commits, rollbacks, tupOps := c.queryThroughput(ctx)
		stmtCalls := c.queryStmtCalls(ctx)
		walBytes := c.queryWALBytes(ctx)

		queries := stmtCalls
		if queries < 0 {
			queries = tupOps // fallback to tuple operations
		}
	...
	odberr.SafeGo(odberr.ErrSentinelCollect, func() {
		defer wg.Done()
		total, active, idle, waiting, activeCPU, activeIO := c.querySessionCounts(ctx)
		...
		snap.ActiveCPUCount = int(activeCPU)
		snap.ActiveIOCount = int(activeIO)
		mu.Unlock()
	})
	...
	// Compute db% and WTR% (available from first frame since they use snapshot counts).
	// db% = active sessions as a percentage indicator (like Oracle AAS).
	snap.DBTimePct = float64(snap.ActiveCount)
	snap.WaitRatio = 0
	if snap.ActiveCount > 0 {
		snap.WaitRatio = float64(snap.WaitingCount) / float64(snap.ActiveCount) * 100
	}
	snap.HasDelta = c.state.Initialized

	// Compute wait event percentages.
	totalActive := 0
	for i := range snap.Events {
		totalActive += snap.Events[i].Sessions
	}
	for i := range snap.Events {
		if totalActive > 0 {
			snap.Events[i].Percentage = float64(snap.Events[i].Sessions) / float64(totalActive) * 100
		}
	}

	// Accumulate cumulative session-counts across frames.
	if c.state.CumulEventSessions == nil {
		c.state.CumulEventSessions = make(map[string]int64)
	}
	for i := range snap.Events {
		key := snap.Events[i].Event
		c.state.CumulEventSessions[key] += int64(snap.Events[i].Sessions)
		snap.Events[i].CumulSessions = c.state.CumulEventSessions[key]
	}
	// Compute cumul percentages.
	var totalCumul int64
	for _, v := range c.state.CumulEventSessions {
		totalCumul += v
	}
	for i := range snap.Events {
		if totalCumul > 0 {
			snap.Events[i].CumulPct = float64(snap.Events[i].CumulSessions) / float64(totalCumul) * 100
		}
	}
...
// queryWALBytes returns wal_bytes from pg_stat_wal (PG14+).
// Gracefully returns 0 if the view does not exist.
func (c *Collector) queryWALBytes(ctx context.Context) int64 {
	// Cache the check result to avoid repeated failures on older PG.
	if c.hasWALStats != nil && !*c.hasWALStats {
		return 0
	}
	res, err := c.driver.Query(ctx, walBytesSQL)
	if err != nil {
		f := false
		c.hasWALStats = &f
		return 0
...
// queryStmtCalls returns the total calls from pg_stat_statements for the current database.
// Returns -1 if pg_stat_statements is not available.
func (c *Collector) queryStmtCalls(ctx context.Context) int64 {
	if c.hasPGStatStmts != nil && !*c.hasPGStatStmts {
		return -1
	}
	res, err := c.driver.Query(ctx, stmtCallsSQL)
	if err != nil {
		f := false
		c.hasPGStatStmts = &f
		return -1
...
// querySessionCounts returns total, active, idle, waiting, activeCPU, activeIO counts.
func (c *Collector) querySessionCounts(ctx context.Context) (total, active, idle, waiting, activeCPU, activeIO int64) {
	res, err := c.driver.Query(ctx, sessionCountSQL)
	if err != nil {
		debugLog("[SESSION] query error: %v", err)
		return 0, 0, 0, 0, 0, 0
	}
	if len(res.Rows) == 0 || len(res.Rows[0]) < 6 {
		return 0, 0, 0, 0, 0, 0
	}
	row := res.Rows[0]
	return toInt(row[0]), toInt(row[1]), toInt(row[2]), toInt(row[3]), toInt(row[4]), toInt(row[5])
}
...
func (c *Collector) queryWaitEvents(ctx context.Context) []WaitEvent {
	...
	for _, row := range res.Rows {
		...
		waitType, _ := row[0].(string)
		event, _ := row[1].(string)
		sessions := int(toInt(row[2]))
...
// Uses query_id on PG14+, falls back to no-query_id SQL on older versions.
func (c *Collector) queryActiveSessions(ctx context.Context) []SessionRow {
	sqlStr := c.pickActiveSessionsSQL()
	res, err := c.driver.Query(ctx, sqlStr)
	if err != nil {
		// If query_id failed, mark unavailable and retry with fallback.
		if c.hasQueryID == nil {
			f := false
			c.hasQueryID = &f
			res, err = c.driver.Query(ctx, activeSessionsSQLNoQID)
			if err != nil || len(res.Rows) == 0 {
				return nil
			}
		} else {
	...
	// Mark query_id as available if first success with the primary SQL.
	if c.hasQueryID == nil {
		t := true
		c.hasQueryID = &t
	}
	...
	for _, row := range res.Rows {
		...
		sessions = append(sessions, SessionRow{
			...
			QueryID:    strVal(row[2]),
			WaitType:   strVal(row[3]),
			Event:      strVal(row[4]),
...
// pickActiveSessionsSQL returns the appropriate SQL based on query_id availability.
func (c *Collector) pickActiveSessionsSQL() string {
	if c.hasQueryID != nil && !*c.hasQueryID {
		return activeSessionsSQLNoQID
	}
	return activeSessionsSQL
}
