 ...
 *
 * advisor.go
 *	  Package sqladvisor provides PostgreSQL SQL execution plan analysis
 ...
 *
 * IDENTIFICATION
 *	  internal/postgres/sqladvisor/advisor.go
 *
 *-------------------------------------------------------------------------
 */
// Package sqladvisor provides PostgreSQL SQL execution plan analysis and optimization recommendations.
package sqladvisor
...
import (
	...
	"github.com/sqlrush/opendb/internal/postgres/sqladvisor/analyzers"
)
...
// New creates a PostgreSQL SQL Advisor with all built-in analyzers.
...
// Analyze performs full analysis on a SQL identified by its queryid.
func (a *Advisor) Analyze(ctx context.Context, queryID string) (*sadv.SQLReport, error) {
	report, err := Collect(ctx, a.driver, queryID)
	if err != nil {
...
func (a *Advisor) FindAndAnalyze(ctx context.Context, text string) ([]*sadv.SQLReport, error) {
	queryIDs, err := FindSQLByText(ctx, a.driver, text)
	if err != nil {
...
func (a *Advisor) AnalyzeTopSQLs(ctx context.Context) ([]*sadv.SQLReport, error) {
	queryIDs, err := FindTopSQLs(ctx, a.driver, 5)
	if err != nil {
