 ...
 *
 * IDENTIFICATION
 *	  internal/postgres/ruleengine/json_provider.go
 *
 *-------------------------------------------------------------------------
 */
package ruleengine
...
func NewJSONRuleProvider(fsys fs.FS, version string) *JSONRuleProvider {
	...
	if err != nil {
		log.Printf("[pg-json-provider] failed to read directory: %v", err)
		return p
	}
...
func NewJSONRuleProviderFromDir(dir string, version string) *JSONRuleProvider {
	...
	if err != nil {
		log.Printf("[pg-json-provider] failed to glob directory: %v", err)
		return p
	}
	...
	if len(p.errors) > 0 {
		log.Printf("[pg-json-provider] loaded %d rules (%d errors) from dir %s",
...
// NewJSONRuleProviderFromEmbedded creates a provider from the embedded PG JSON rules.
