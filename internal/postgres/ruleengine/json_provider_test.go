 ...
 *
 * IDENTIFICATION
 *	  internal/postgres/ruleengine/json_provider_test.go
 *
 *-------------------------------------------------------------------------
 */
package ruleengine
...
import (
	...
	"github.com/sqlrush/opendb/internal/postgres/ruleengine/rules_json"
...
func TestJSONProviderLoadsAllRules(t *testing.T) {
	...
	// PG JSON rules: some have parse errors (decision_tree array format).
...
// ─── Integration Test: PG Idle in Transaction ─────────────────────────────

func TestPGEngineIdleInTransaction(t *testing.T) {
	community := &CommunityProvider{}
	json := NewJSONRuleProvider(rules_json.RuleFiles, "test-1.0")
	combined := NewCombinedProvider(community, json)
...
// ─── Integration Test: PG Lock Waits ──────────────────────────────────────

func TestPGEngineLockWaits(t *testing.T) {
	community := &CommunityProvider{}
	json := NewJSONRuleProvider(rules_json.RuleFiles, "test-1.0")
	combined := NewCombinedProvider(community, json)
...
// ─── Integration Test: PG Vacuum Lag ──────────────────────────────────────

func TestPGEngineVacuumLag(t *testing.T) {
	community := &CommunityProvider{}
	json := NewJSONRuleProvider(rules_json.RuleFiles, "test-1.0")
	combined := NewCombinedProvider(community, json)
	...
	report := mkReport(map[string]sentinel.MetricSummary{
		"dead_tuple_ratio": {Avg: 15, Max: 25, Trend: "rising"},
		"active_sessions":  {Avg: 5, Max: 8, Trend: "stable"},
		"xid_age_pct":      {Avg: 30, Max: 40, Trend: "rising"},
	})
	report.Classification.Cause = sentinel.CauseVacuumLag
	...
	if output.Primary == nil {
		t.Fatal("expected a primary diagnosis for vacuum lag, got nil")
	}
...
// ─── Integration Test: PG Low Load (no match expected) ────────────────────

func TestPGEngineLowLoad(t *testing.T) {
	community := &CommunityProvider{}
	json := NewJSONRuleProvider(rules_json.RuleFiles, "test-1.0")
	combined := NewCombinedProvider(community, json)
