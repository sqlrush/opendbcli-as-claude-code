 ...
 *
 * IDENTIFICATION
 *	  internal/postgres/ruleengine/engine.go
 *
 *-------------------------------------------------------------------------
 */
package ruleengine
...
type EvalContext struct {
	...
	TopSQLs        []sentinel.SQLProfile             // top active queries
	WaitProfile    []sentinel.WaitBucket             // wait event distribution
	BlockingChains []sentinel.BlockingChain          // blocker → victim chains
...
// GetWaitPct returns the percentage for a given wait_event or wait_event_type.
// Searches WaitProfile by exact event name first, then by event type.
// Returns 0 if not found.
func (ctx *EvalContext) GetWaitPct(event string) float64 {
	// Exact match on wait_event
	for _, b := range ctx.WaitProfile {
		if b.WaitEvent == event {
			return b.Percentage
		}
	}
	// Match on wait_event_type (aggregate)
	var total float64
	for _, b := range ctx.WaitProfile {
		if b.WaitEventType == event {
			total += b.Percentage
		}
	}
	return total
...
// WaitAvgMs returns 0 for PG (wait profile is count-based, not time-based).
...
// Engine is the deterministic, decision-tree-based diagnosis engine for PostgreSQL.
type Engine struct {
	rules    []*Rule
	index    *SignalIndex
	config   Config
...
func extractSignals(input *DiagInput) []Signal {
	...
	case InputHealthCheck:
		// Health check matches all PG category signals
...
// extractBurstSignals extracts signals from a PG sentinel BurstReport.
func extractBurstSignals(input *DiagInput) []Signal {
	...
	// ── Metric signals ──
	// For PG, emit signals for all non-zero metrics (JSON rules use metric names
	// as signals, and live report metrics have no trend data).
	...
	// ── WaitEvent signals from WaitProfile ──
	for _, bucket := range report.WaitProfile {
		if bucket.WaitEventType != "" && bucket.WaitEventType != "CPU" {
			signals = append(signals, Signal{
				Type: SignalWaitEvent,
				Key:  bucket.WaitEventType,
			})
		}
		if bucket.WaitEvent != "" && bucket.WaitEvent != "Running" {
			signals = append(signals, Signal{
				Type: SignalWaitEvent,
				Key:  bucket.WaitEvent,
			})
		}
	}
	...
	// ── Selective category signals from metrics ──
	// Only add categories that are directly supported by anomalous metrics.
	// Do NOT blanket-add all categories — this causes unrelated rules (e.g.,
	// high_availability/repmgr) to match every report.
	// The inferCategoriesFromMetrics call above already handles precise mapping.
	// Add a few more cross-cutting categories when evidence exists:
	if m, ok := report.Metrics[string(sentinel.MetricActiveSessions)]; ok && m.Max > 10 {
		addCategory("performance")
	}
	if m, ok := report.Metrics[string(sentinel.MetricTupReturnedRate)]; ok && m.Max > 100000 {
		addCategory("query_optimization")
	}
	if m, ok := report.Metrics[string(sentinel.MetricAutovacuumWorkers)]; ok && m.Max > 0 {
		addCategory("maintenance")
	}
	if m, ok := report.Metrics[string(sentinel.MetricLockWaitSessions)]; ok && m.Max > 0 {
		addCategory("concurrency")
	}
	if m, ok := report.Metrics[string(sentinel.MetricBackendErrorRate)]; ok && m.Max > 0 {
...
func isPercentageMetric(name string) bool {
	return strings.HasSuffix(name, "_pct") ||
		...
		strings.Contains(name, "_hit_")
}

// inferCategoriesFromMetrics derives category signals from PG metric patterns.
// PG doesn't have Oracle wait events; instead we use metric names to infer categories.
func inferCategoriesFromMetrics(metrics map[string]sentinel.MetricSummary, addCategory func(string)) {
	for name, m := range metrics {
		...
		switch {
		case name == "lock_waits" || name == "lock_wait_sessions" || name == "blocker_count" || name == "deadlocks":
			if m.Max > 0 {
				addCategory("lock")
				addCategory("concurrency")
			}
		case name == "connections_pct":
			if m.Max > 50 {
				addCategory("connection")
			}
		case name == "idle_in_transaction":
			if m.Max > 3 {
				addCategory("connection")
			}
		case name == "lwlock_wait_sessions":
			if m.Max > 0 {
				addCategory("io_storage")
			}
		case name == "bufferpin_wait_sessions":
			if m.Max > 0 {
				addCategory("io_storage")
			}
		case name == "tablespace_used_pct":
			if m.Max > 80 {
				addCategory("io_storage")
				addCategory("storage")
			}
		case name == "io_wait_sessions":
			if m.Max > 3 {
				addCategory("io_storage")
			}
		case name == "backend_errors_rate" || name == "backend_error_rate":
			if m.Max > 0 {
				addCategory("monitoring")
			}
		case name == "dead_tuple_ratio":
			if m.Max > 5 {
				addCategory("vacuum")
			}
		case name == "xid_age_pct":
			if m.Max > 50 {
				addCategory("vacuum")
			}
		case name == "wal_bytes_rate":
			if m.Max > 10000000 { // > 10MB/s
				addCategory("wal")
			}
		case name == "checkpoints_req":
			if m.Max > 3 { // > 3 per hour
				addCategory("wal")
				addCategory("checkpoint")
			}
		case name == "replication_lag_sec":
			if m.Max > 0 {
				addCategory("replication")
			}
		case name == "cache_hit_pct":
			if m.Max > 0 && m.Min < 95 {
				addCategory("memory")
			}
		case name == "temp_bytes_rate":
			if m.Max > 0 {
				addCategory("io_storage")
			}
		case name == "long_queries":
			if m.Max > 0 {
...
// extractErrorSignals extracts signals from a PG error code (SQLSTATE).
...
// rootCauseToCategory maps PG sentinel root cause types to rule engine categories.
...
// walkTreeElimination evaluates all branches to implement 排除法.
