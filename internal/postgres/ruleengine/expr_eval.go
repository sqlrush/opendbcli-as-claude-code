 ...
 *
 * IDENTIFICATION
 *	  internal/postgres/ruleengine/expr_eval.go
 *
 *-------------------------------------------------------------------------
 */
package ruleengine
...
type ExprCheck struct {
	Source   string // "metrics", "summary", "query_result"
	Field    string // metric name, query field
	QueryRef string // reference to diagnostic_queries key
}
...
// ParseConfidence extracts a confidence value from a string like "置信度 92%".
