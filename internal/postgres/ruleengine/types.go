 ...
 *	  database diagnosis engine for PostgreSQL. It works without LLM and
 ...
 *
 * IDENTIFICATION
 *	  internal/postgres/ruleengine/types.go
 *
 *-------------------------------------------------------------------------
 */
// Package ruleengine provides a deterministic, decision-tree-based database
// diagnosis engine for PostgreSQL. It works without LLM and serves as fallback
// when LLM chain-of-thought fails to identify the root cause.
package ruleengine
...
const (
	SignalWaitEvent SignalType = iota // PG wait_event name
	SignalErrorCode                   // PG error code (SQLSTATE)
...
type Signal struct {
	Type SignalType
	Key  string // e.g., "LWLock", "lock_waits", "active_sessions"
}
...
type Condition struct {
	Source string  // "metrics" / "summary"
	Field  string  // metric name, etc.
...
type Action struct {
	...
	SkillCommand string // opendb skill command
	RawSQL       string // raw SQL
...
type Rule struct {
	ID       string
	Name     string
	Category string // "vacuum" / "lock" / "wal" / "connection" / "replication" / "checkpoint" / "sql_perf" / "memory" / "io_storage"
...
// RuleProvider supplies rules to the engine.
...
const (
	...
	InputErrorCode                    // specific PG error (SQLSTATE)
