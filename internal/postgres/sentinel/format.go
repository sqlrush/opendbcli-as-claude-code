 ...
 *
 * IDENTIFICATION
 *	  internal/postgres/sentinel/format.go
 *
 *-------------------------------------------------------------------------
 */
package sentinel
...
// formatTriggerReason returns the detection strategy description.
...
func FormatRuleDiagnosis(report BurstReport) string {
	...
	b.WriteString("  PG Sentinel 异常报告\n\n")
	...
	// Metric summary (use priority order).
	...
	if len(report.SpaceDetails) > 0 {
		...
		for _, sd := range report.SpaceDetails {
			if sd.UsedMB > 0 {
				b.WriteString(fmt.Sprintf("    %s: %.1f MB (%.1f%%)", sd.Name, sd.UsedMB, sd.UsedPct))
			} else {
				b.WriteString(fmt.Sprintf("    %s: %.1f%%", sd.Name, sd.UsedPct))
			}
			if sd.Extra != "" {
