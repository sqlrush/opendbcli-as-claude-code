 ...
 *
 * sqladvisor_skill.go
 *	  SQLAdvisorSkill provides MySQL SQL performance diagnosis and
 ...
 *
 * IDENTIFICATION
 *	  internal/mysql/skill/query/sqladvisor_skill.go
 *
 *-------------------------------------------------------------------------
 */
package query
...
// SQLAdvisorSkill provides MySQL SQL performance diagnosis and optimization suggestions.
...
func (s *SQLAdvisorSkill) Description() string                { return "MySQL SQL 性能诊断与优化建议" }
...
func (s *SQLAdvisorSkill) ToolDef() skill.ToolDef {
	return skill.ToolDef{
		Name:        "sqladvisor",
		Description: "Analyze MySQL SQL execution plan and provide optimization suggestions",
		Parameters: map[string]any{
			"type": "object",
			"properties": map[string]any{
				"args": map[string]any{
					"type":        "string",
					"description": "SQL digest, SQL text fragment, or empty for auto top SQL analysis",
				},
			},
		},
	}
...
func (s *SQLAdvisorSkill) CLIDef() skill.CLIDef {
	return skill.CLIDef{
		...
		Usage:   "/sqladvisor [digest | \"SQL text\"]",
		Examples: []string{
			"/sqladvisor",
			"/sqladvisor abc123def456",
...
func (s *SQLAdvisorSkill) Execute(ctx context.Context, params skill.Params) (*skill.Result, error) {
	...
	advisor := mysqlsqladv.New(s.driver)
