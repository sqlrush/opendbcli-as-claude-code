 ...
 *
 * IDENTIFICATION
 *	  internal/mysql/skill/monitor/os.go
 *
 *-------------------------------------------------------------------------
 */
package monitor
...
func (s *OSSkill) Category() string                   { return "monitor" }
...
func (s *OSSkill) Execute(_ context.Context, _ skill.Params) (*skill.Result, error) {
	...
	// Load average
	if loadLines, err := readLoadAvg(); err == nil {
	...
	// Memory
	if memLines, err := readMemInfo(); err == nil {
		sections = append(sections, format.PanelSection{
			Header: "Memory",
			Lines:  memLines,
		})
	...
	// CPU
	if cpuLines, err := readCPUInfo(); err == nil {
		sections = append(sections, format.PanelSection{
			Header: "CPU",
			Lines:  cpuLines,
		})
