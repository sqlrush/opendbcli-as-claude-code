 ...
 *
 * IDENTIFICATION
 *	  internal/oracle/skill/monitor/dbtop.go
 *
 *-------------------------------------------------------------------------
 */
package monitor
...
func (c *DbtopRefreshConfig) NewDbtopLoop() skill.DbtopLoop {
	return &oracleDbtopLoop{collector: dbtop.NewCollector(c.Driver)}
}

// oracleDbtopLoop wraps the Oracle collector+renderer into a DbtopLoop.
type oracleDbtopLoop struct {
	collector *dbtop.Collector
}

func (l *oracleDbtopLoop) RenderFrame(ctx context.Context, cols, intervalSec int) []string {
	snap := l.collector.Collect(ctx)
	return dbtop.Render(snap, cols, intervalSec)
}

// DBTopSkill provides a real-time database monitoring dashboard.
...
func (s *DBTopSkill) Description() string               { return "Real-time database performance monitor" }
...
func (s *DBTopSkill) ToolDef() skill.ToolDef {
	return skill.ToolDef{
		Name:        "dbtop",
		Description: "Real-time database performance monitor",
	}
}
