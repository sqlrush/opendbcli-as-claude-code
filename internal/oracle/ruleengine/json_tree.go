 ...
 *
 * json_tree.go
 *	  BuildTreeFromJSON converts a JSONTreeNode to a TreeNode. queryMap
 *	  maps JSON query key names to registered QueryIDs. rootCauses maps
 *	  RC IDs (e.g., "RC1") to JSONRootCause for enrichment.
 *
 *
 ...
 *
 * IDENTIFICATION
 *	  internal/oracle/ruleengine/json_tree.go
 *
 *-------------------------------------------------------------------------
 */
package ruleengine
...
// queryMap maps JSON query key names to registered QueryIDs.
// rootCauses maps RC IDs (e.g., "RC1") to JSONRootCause for enrichment.
...
func buildTreeNode(
	...
	// Set elimination method flag
	node.EliminationMethod = jnode.EliminationMethod

	// Set query if referenced
	if jnode.Query != "" {
...
func convertBranch(
	...
	// Convert condition → Match closure
	...
	// Convert cross_reference as findings
	...
	// Recursively convert Then subtree
	if jb.Then != nil {
...
func buildMatchFunc(cond JSONBranchCond) func(interface{}) bool {
	...
	// No field+op means no condition specified — treat as default
	...
	// Check if this is a semantic boolean condition (e.g., "clear_anomaly" eq true).
	// Semantic fields are abstract LLM markers that don't exist in query results.
	// Heuristic: if the field can't be found in the value but value is non-empty,
	// treat query-returned data as confirmation of the positive path.
	if isSemanticBoolField(cond.Field) && op == "eq" && isTrueish(cond.Value) {
		field := cond.Field
		return func(value interface{}) bool {
			// If the field actually exists in value, use real evaluation
			if extracted := extractField(value, field); extracted != nil {
				return EvalOp(op, extracted, true)
			}
			// Heuristic: query returned data → anomaly is confirmed → positive branch
			return isNonEmptyValue(value)
		}
	}

	// Standard declarative match
	expr := &ExprMatch{
		IsDefault: false,
		Field:     cond.Field,
		Op:        op,
...
// rather than a real database metric or query result column.
func isSemanticBoolField(field string) bool {
	// Known semantic field patterns from ailinkdb JSON rules
...
func isNonEmptyValue(value interface{}) bool {
	...
	case string:
		return v != ""
	default:
		return true // non-nil = something exists
	}
}
