 ...
 *
 * IDENTIFICATION
 *	  internal/oracle/ruleengine/index.go
 *
 *-------------------------------------------------------------------------
 */
package ruleengine
...
// Each rule's declared signals are indexed by type and key.
	...
	for _, rule := range rules {
		for _, sig := range rule.Signals {
			...
			case SignalErrorCode:
				// Error codes stored in uppercase for canonical form
...
// Rules are returned in no particular order; the caller is responsible for ranking.
func (idx *SignalIndex) Match(signals []Signal) []*Rule {
	seen := make(map[string]bool) // rule ID -> already added
	var candidates []*Rule
...
// This allows user questions to match rules even when the keyword is
// a substring of the indexed key or vice versa.
func (idx *SignalIndex) matchKeywords(keyword string) []*Rule {
	...
	// Substring match: check if the keyword contains or is contained by any indexed key
	var matched []*Rule
