 ...
 *
 * IDENTIFICATION
 *	  internal/oracle/ruleengine/json_query_registry.go
 *
 *-------------------------------------------------------------------------
 */
package ruleengine
...
// Returns the registered QueryIDs keyed by their original query key.
...
//
// Wraps the base QueryExecutor to support both predefined and dynamic queries.
...
// Execute runs a query by its ID. For dynamic queries (json:*), it looks up
// the SQL from the registry. For predefined queries, it uses the queryRegistry.
func (e *DynamicQueryExecutor) Execute(qid QueryID, params map[string]string) (interface{}, error) {
	...
	if IsDynamicQueryID(qid) {
		...
		// Predefined query from queryRegistry
	...
	// Substitute parameters (e.g., {sql_id} → actual value)
...
// that decision trees can inspect.
func queryResultToMap(qr *db.QueryResult) interface{} {
	if qr == nil || len(qr.Rows) == 0 {
		return nil
	}

	// Single row → map[string]interface{}
	if len(qr.Rows) == 1 {
	...
	// Multiple rows → []map[string]interface{}
