 ...
 *
 * IDENTIFICATION
 *	  internal/oracle/sentinel/detector.go
 *
 *-------------------------------------------------------------------------
 */
package sentinel
...
func (h *MetricHistory) Values() []float64 {
	...
	if h.full {
		// pos points to the oldest element
...
func (d *DetectorState) CheckT3(metric MetricName, bl *MetricBaseline, cfg Config) *TriggerEvent {
	...
	trendWindows := GetMetricDef(metric).TrendWindows
	if trendWindows <= 0 {
		trendWindows = 3
	}
...
func (d *DetectorState) CheckT4(metric MetricName, current float64, bl *MetricBaseline, hw HardwareProfile) *TriggerEvent {
	...
	vals := h.Values()
	n := len(vals)
	// 2nd derivative: v[n] - 2*v[n-1] + v[n-2]
...
// Special case: instance_status triggers when value != 1 (non-OPEN).
func (d *DetectorState) CheckT5(metric MetricName, current float64, def *MetricDef, bl *MetricBaseline) *TriggerEvent {
	if def == nil {
		return nil
	}

	// Special case: instance_status triggers on non-OPEN (value != 1).
	if metric == MetricInstanceStatus {
		if current == 1 {
			return nil
		}
		return &TriggerEvent{
...
// For inverted metrics (free %): current <= CapacityRed.
func (d *DetectorState) CheckT6(metric MetricName, current float64, def *MetricDef) *TriggerEvent {
	...
	// Red threshold — no sustained count needed, trigger immediately.
	return &TriggerEvent{
		Timestamp:  time.Now(),
		Metric:     string(metric),
...
func (d *DetectorState) CheckT7(metric MetricName, bl *MetricBaseline) *TriggerEvent {
	...
	oldMean := mean(oldHalf)
	newMean := mean(newHalf)
	oldStd := stddev(oldHalf, oldMean)
...
// latch_free_rate: trigger if current > RegressionFloor (miss rate too high).
// Hit ratios / free %: trigger if current < RegressionFloor (value too low).
func (d *DetectorState) CheckT8(metric MetricName, current float64, def *MetricDef) *TriggerEvent {
	...
	// Determine direction: latch_free_rate is a "miss" metric — higher is worse.
	// All others (buffer_cache_hit, library_cache_hit, shared_pool_free) — lower is worse.
	var regressed bool
	if metric == MetricLatchFreeRate {
...
// (indicating a legitimate business slowdown rather than an anomaly).
func (d *DetectorState) CheckT9(
	...
	// Skip if baseline too low to be meaningful.
	if bl.Avg < def.MinBaseline {
		d.sustainedT9[metric] = 0
		return nil
	}

	// Check if current dropped below threshold.
	...
	if def.CheckActiveSync {
		activeCurrent, hasActive := sample.Values[MetricActive]
		if hasActive {
			// Look up active_sessions baseline from the history.
			activeH, hasHistory := d.histories[MetricActive]
			if hasHistory && activeH.Len() > 0 {
				activeVals := activeH.Values()
				activeBaseline := mean(activeVals)
				if activeBaseline > 0 {
					activeRatio := activeCurrent / activeBaseline
					// If active also dropped proportionally (< 0.5), it's a business lull.
					if activeRatio < 0.5 {
						d.sustainedT9[metric] = 0
						return nil
					}
...
// T1 and T2 are handled separately in sentinel.go.
func (d *DetectorState) DetectExtended(sample SentinelSample, baseline Baseline, cfg Config, suppressed ...map[MetricName]float64) *TriggerEvent {
	var suppressedSet map[MetricName]float64
	if len(suppressed) > 0 {
		suppressedSet = suppressed[0]
	}

	for _, metric := range metricPriorityOrder {
		// Skip suppressed metrics (already triggered, not yet recovered).
		if suppressedSet != nil {
			if _, ok := suppressedSet[metric]; ok {
				continue
			}
		}
		...
		// Push value into history for this metric.
...
// stddev computes the population standard deviation given a precomputed mean.
